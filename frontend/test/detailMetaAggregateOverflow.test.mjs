...
const displaySource = fs.readFileSync(new URL('../src/modules/design-system/pages/sections/DisplaySection.tsx', import.meta.url), 'utf8');
...
test('详情长文本保留手工回车并完整换行', () => {
  assert.match(detailMetaSource, /item\.longText[\s\S]*is-long-text/);
  assert.match(detailMetaStyles, /\.admin-detail-meta-list__item dd\.is-long-text[\s\S]*white-space:\s*pre-wrap/);
  assert.match(detailMetaStyles, /\.admin-detail-meta-list__item dd\.is-long-text[\s\S]*overflow-wrap:\s*anywhere/);
  assert.match(displaySource, /备注换行示例[\s\S]*\\n[\s\S]*longText:\s*true/);
  assert.match(developmentRules, /longText[\s\S]*保留手工回车/);
});
